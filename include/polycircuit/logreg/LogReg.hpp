...
class LogReg final : public IComponent {
public:
    explicit LogReg(lbcrypto::CryptoContext<ElementType>&& cc, lbcrypto::Ciphertext<ElementType>&& inputC)
        : m_cc(std::move(cc)), m_inputC(std::move(inputC)) {}
    explicit LogReg(const lbcrypto::CryptoContext<ElementType>& cc, const lbcrypto::Ciphertext<ElementType>& inputC)
        : m_cc(cc), m_inputC(inputC) {}
    LogReg(const LogReg&)            = default;
    LogReg(LogReg&&)                 = default;
    LogReg& operator=(const LogReg&) = default;
    LogReg& operator=(LogReg&&)      = default;
    ...
    Ciphertext evaluate() override {
        ...
        temp1 =
            m_cc->EvalMult(m_cc->EvalMult(m_inputC, (coeff * 32 * 4) / (std::pow(25, 3))), m_cc->EvalSquare(m_inputC));
