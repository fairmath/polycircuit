...
namespace polycircuit {
    namespace ckks {
        ...
        namespace {
            ...
            std::shared_ptr<lbcrypto::longDiv>
            LongDivisionPolyModQ(const std::vector<double> &f, const std::vector<double> &g,
                                 const double q = 786433)  // 786433 //65537
            {
                ...
                if (n != f.size() - 1) {
                    throw std::invalid_argument(
                            "LongDivisionPolyModQ: The dominant coefficient of the divident is zero.");
                }

                if (k != g.size() - 1) {
                    throw std::invalid_argument(
                            "LongDivisionPolyModQ: The dominant coefficient of the divisor is zero.");
                }
            ...
            MultByInteger(const lbcrypto::Ciphertext<ElementType> &ct, const uint64_t scalar) {
            ...
            template<typename ElementType>
            lbcrypto::Ciphertext<ElementType>
            EvalAdd(const lbcrypto::Ciphertext<ElementType> &ct, const int64_t scalar) {
                const auto cc = ct->GetCryptoContext();
            ...
            lbcrypto::Ciphertext<ElementType> EvalPolyPS(lbcrypto::ConstCiphertext<ElementType> x,
                ...
                const auto cryptoParams =
                        std::dynamic_pointer_cast<lbcrypto::CryptoParametersCKKSRNS>(
                                powers[k - 1]->GetCryptoParameters());
