...
class SHL final : public IComponent
{
public:
    explicit SHL(lbcrypto::CryptoContext<ElementType>&& cc,
        lbcrypto::Ciphertext<ElementType>&& indexC,
        lbcrypto::Ciphertext<ElementType>&& inputC)
        : m_cc(std::move(cc))
        , m_indexC(std::move(indexC))
        , m_inputC(std::move(inputC))
    { }
    explicit SHL(const lbcrypto::CryptoContext<ElementType>& cc,
...
std::shared_ptr<lbcrypto::longDiv> LongDivisionPolyModQ(
...
template <typename ElementType>
lbcrypto::Ciphertext<ElementType> MultByInteger(
    const lbcrypto::Ciphertext<ElementType>& ct,
...
template <typename ElementType>
lbcrypto::Ciphertext<ElementType> EvalLinearWSum(
    const std::vector<lbcrypto::Ciphertext<ElementType>>& ctxs,
...
template <typename ElementType>
lbcrypto::Ciphertext<ElementType> EvalAdd(
    const lbcrypto::Ciphertext<ElementType>& ct,
    const int64_t scalar)
{
    const auto cc = ct->GetCryptoContext();
...
template <typename ElementType>
lbcrypto::Ciphertext<ElementType> InnerEvalPolyPS(
    lbcrypto::ConstCiphertext<ElementType> x,
...
template <typename ElementType>
lbcrypto::Ciphertext<ElementType> EvalPolyPS(
    lbcrypto::ConstCiphertext<ElementType> x,
    const std::vector<double>& coefficients)
{
    ...
    const auto cryptoParams = std::dynamic_pointer_cast<lbcrypto::CryptoParametersCKKSRNS>(powers[k - 1]->GetCryptoParameters());
...
Ciphertext SHL<ElementType>::evaluate()
{
    ...
    std::vector<double> coef = { 1, 10802, 112723, 301260, 147254, 278067, 583275, 252765, 146881, 346114, 251411, 552443, 335648, 343498, 206133, 63890 };
