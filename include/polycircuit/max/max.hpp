...
namespace polycircuit {
    namespace ckks {
        ...
        public:
            ...
            explicit matrxi_mul(const lbcrypto::CryptoContext<ElementType> &cc,
                                const lbcrypto::Ciphertext<ElementType> &arrayC,
                                const std::vector<std::vector<int64_t>> &coeffs, const size_t arraySize,
                                const int infimum)
        ...
        namespace {
            ...
            template<typename ElementType>
            lbcrypto::Ciphertext<ElementType>
            duplicateCiphertext(const lbcrypto::Ciphertext<ElementType> &x, size_t arraySize) {
...
// Collect the x^d monomial coefficients for multiple output polynomials.
            std::vector<int64_t>
            getCoefficientSlice(const std::vector<std::vector<int64_t>> &coeffs, const size_t numElts,
...
#pragma omp parallel for
            ...
            template<typename ElementType>
            lbcrypto::Ciphertext<ElementType>
            EvalNandDensity(lbcrypto::Ciphertext<ElementType> x, const size_t arraySize) {
                lbcrypto::CryptoContext<ElementType> cc = x->GetCryptoContext();
                size_t r = 1;
                while (r < arraySize) {
...
// processing. Note: this modifies results in-place.
            template<typename ElementType>
            lbcrypto::Ciphertext<ElementType>
            parallelAggregate(std::vector<lbcrypto::Ciphertext<ElementType>> &results) {
